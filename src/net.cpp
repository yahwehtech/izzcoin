...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "IZZcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"explorer.izzcoin.com", "explorer.izzcoin.com"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. IZZcoin is probably already running."), addrBind.ToString().c_str());
        else
