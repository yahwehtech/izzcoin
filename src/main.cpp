...
#if defined(NDEBUG)
# error "IZZcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x017024b22be4258fc067386b0793f28b7035a56a720281709d9f2821bcaee252");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IZZcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IZZcoin Signed Message:\n";
...
{
    // IZZcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan =  12 * 60 * 60; // IZZcoin: 12 hours 
static const int64 nTargetSpacing =  60; // IZZcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // IZZcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IZZcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x017024b22be4258fc067386b0793f28b7035a56a720281709d9f2821bcaee252");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "27th izzcoin birthday";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042642710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1530082210;
        ...
        assert(block.hashMerkleRoot == uint256("0x3b35187b998883227be260ade7473dcfb5ced9dc34f2563646a857ff6a0d1a1a"));
...
unsigned char pchMessageStart[4] = { 0xd2, 0xc0, 0xd1, 0xbd }; // IZZcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IZZcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IZZcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IZZcoinMiner(CWallet *pwallet)
{
    printf("IZZcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("izzcoin-miner");
    ...
    try { loop {
        ...
        printf("Running IZZcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IZZcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IZZcoinMiner, pwallet));
}
